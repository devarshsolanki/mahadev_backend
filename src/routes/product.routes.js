...
const createProductValidation = [
  ...
  body('mrp').toFloat().isFloat({ min: 0 }).withMessage('Valid MRP is required'),
  body('stock').toInt().isInt({ min: 0 }).withMessage('Stock must be a positive number'),
...
const updateProductValidation = [
  ...
  body('price').optional().toFloat().isFloat({ min: 0 }).withMessage('Valid price is required'),
  body('mrp').optional().toFloat().isFloat({ min: 0 }).withMessage('MRP must be a positive number'),
  body('unit').optional().trim().notEmpty().withMessage('Unit cannot be empty'),
  body('stock').optional().toInt().isInt({ min: 0 }).withMessage('Stock must be a positive number'),
  body('isFeatured').optional().custom(val => {
    // Accept 'true', 'false', true, false as valid boolean values
    if (typeof val === 'string') {
      return ['true', 'false'].includes(val.toLowerCase());
